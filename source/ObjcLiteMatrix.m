...
- (instancetype)initWithRowSize:(NSInteger)rowSize
                 withColumnSize:(NSInteger)colSize {
  ...
  if (self) {
    ...
    // dynamically allocate the C-style array used to to hold dumb pointers to
    // objects
    ...
    _arrayOfObjects =
        [[NSMutableArray alloc] initWithCapacity:_rowCapacity * _colCapacity];
  }
...
- (void)addObjectToMatrixAtIndex:(id)object
                             row:(NSInteger)row
